...
function hybrid_media_grabber( $args = array() ) {
	...
	return $media->get_media();//$media->get_media();
}
...
class Hybrid_Media_Grabber {
	...
	public function get_media() {
		return $this;//apply_filters( 'hybrid_media_grabber_media', $this->media, $this );
	}
	...
	public function do_embed_shortcode_media( $shortcode ) {
		...
		$this->media = do_shortcode(apply_filters(
	...
	public function do_autoembed_media() {
		...
		switch ( $ext )
		{
			case 'mp3' : case 'ogg' : case 'm4a' : case 'm4v' : case 'ogv' : case 'webmv' :
				$this->spit = false;
				$this->media = $value[0];
			break;
		}
...
//echo $embed;
	...
	public function do_attached_media() {
		...
		if ( $attached_media ) {
			...
			$this->media = $url;//do_shortcode( "[{$this->type} src='{$url}']" );
			$this->spit = false;
		}
	}
